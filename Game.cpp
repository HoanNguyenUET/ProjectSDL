...
void Game::init(const char *title, int xpos, int ypos, int width, int height, bool fullscreen)
{
    ...
    gameoverBackground = TextureManager::LoadTexture("pics/endbackground.png",renderer);
}
...
void Game::update()
{
    ...
    {
        score += 1;
        nextTimeIncreaseScore = SDL_GetTicks64() + 500;
    }
        ...
        if(t->xpos + t->width - 15 >= player->xpos &&
            ...
            )
            {
                ...
                if(player->health == 0)
                {
                    isRunning = false;
                    isGameOver = true;
                    std::string highscore = "Your score: " + std::to_string(score);
                    highscoreText = new Text("font/CastoroTitling-Regular.ttf",highscore.c_str(), 50, 200, 300, SDL_Color{0,0,0});
                    highScoreInp.open("highscore.txt");
                    int bestHighScore;
                    highScoreInp >> bestHighScore;
                    if(bestHighScore < score)
                    {
                        bestHighScore = score;
                    }
                    highScoreInp.close();
                    ...
                    highScoreOut << bestHighScore;
                    highScoreOut.close();
                    std::string bestHighScoreStr = "Highscore :" + std::to_string(bestHighScore);
                    bestScore = new Text("font/CastoroTitling-Regular.ttf",bestHighScoreStr.c_str(), 50, 200, 400, SDL_Color{0,0,0});
